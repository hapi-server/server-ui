...
function getFiles(files, options, cb) {
  ...
  if (files.length === 0) {
    let err = new Error("No files to read.");
    cb(err, null);
    return;
  }
  ...
  for (let [i, file] of Object.entries(files)) {
    if (typeof file !== 'string') {
      let err = new TypeError("Array elements of 'files' must strings. Element " + i + " is a " + typeof(file) + ".");
      cb(err, null);
    }
    getFile(file, cb);
  }
  ...
  function getFile(file) {
    ...
    if (file.startsWith("http") || !isFile) {
      ...
      let textMimeTypes = ["json","text","xml"];
      ...
      fetch(file)
        .then(function (res) {
          ...
          if (contentType) {
            for (let textMimeType of textMimeTypes) {
