...
function initAll(app, opts) {
  ...
  let allFiles = checkInput(opts["allFiles"] ? opts["allFiles"] : [], 'allFiles');
  let allArray = checkInput(opts["allArray"] ? opts["allArray"] : [], 'allArray');
  ...
  if (allFiles.length === 0 && allArray.length === 0) {
    let err = new TypeError("At least one of allFiles and allArray must be non-empty.");
    console.error(err);
    process.exit(1);
  }
      ...
      for (let [file, result] of Object.entries(results)) {
        if (result.error) {
          ...
          if (processFileResponses["allLast"][file] !== undefined) {
            ...
            let lastDate = processFileResponses["allLast"][file]['date'];
            let allObj = {
