function data() {
  ...
  get({url: url, timer: {'element': '#dataRequestTiming'}, ...getOptions},
    function (err, data, length, nRecords) {
      if (err) {
        let msg = `Error when requesting ${html.aLink(url,'data')}: ${err}.`
      ...
      let msg = `(${nRecords} records, ${util.bytesWithSIUnits(length)})`;
