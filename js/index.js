...
function main() {
    ...
    $('#plotserver').change(function (){
        if (qs["return"] && qs["return"] === "image" && qs["format"]) {
            console.log("plotserver changed. Triggering select event on #format drop-down.");
...
function hashchange() {
    ...
    $(window).bind("hashchange", function() {
        ...
        // updated.
        console.log("hashchange(): Hash change; by url edit = "
                    + $(window).hashchange.byurledit);
        var qs = parseQueryString();

        let Nchanged = 0;
        for (var id in qs) {
            console.log("hashchange(): Value in query string = " + qs[id]);
            var val = $('#' + id).parent().parent().attr('value');
            console.log("hashchange(): Drop-down value  = " + val);
            if (qs[id] !== val) {
                Nchanged = Nchanged + 1;
                console.log('hashchange(): Query string value differs from that in ' + id + " dropdown.");
            } else {
                console.log('hashchange(): Query string value same as that in ' + id + " dropdown.");           
            }
        }

        if (Nchanged > 1) {
            console.log('hashchange(): More than one query string value changed. Resetting app.');
            $(window).unbind("hashchange");
            location.reload();
            return;
        } else {                        
            console.log('hashchange(): No query string value changed.');
        }

        if ($(window).hashchange.byurledit) {
            console.log("hashchange(): Hash change made by manual edit of one parameter.");
            console.log("hashchange(): Last hash:");
            console.log(location.hash);
            for (var id in qs) {
                console.log("hashchange(): Setting " + id + " drop-down to " + qs[id] + " and triggering change.");
...
function servers(cb) {
    ...
    var SERVER_LIST_HASH = server_list_in_hash();
    if (SERVER_LIST_HASH !== "") {
        ...
        get({url: SERVER_LIST, showAjaxError: false}, function (err, res) {
            ...
            if (err) {
                ...
                $('#xstatus').append("<span style='background-color:yellow'>" + warning + "</span>");
...
function checktimes(which) {
    if (selected('start') && selected('stop')) {
        console.log("stoptime select event.")
        console.log("starttime = " + selected('start'))                 
        console.log("stoptime = " + selected('stop'))
        var t = dayjs(selected('start').replace("Z","")) 
                < dayjs(selected('stop').replace("Z",""));
        console.log("---> start < stop? " + t);
        if (t == false) {
            console.log(which + " changed; start >= stop. Setting color of " + which + " to red.");
            $('#' + which + 'list').css('color','red');
            return "Error";
        } else {
            console.log(which + " changed; start < stop. Setting color of " + which + " to black.");
